...
_current = [""]  # sentence currently being spoken (echo guard for "stop" words)
...
def _pronounce(text: str) -> str:
    """Kokoro reads [word](/phonemes/) literally — use it to say the name our way."""
    ph = _load_config()["tts"].get("name_phonemes")
    return re.sub(r"\bSebastian\b", f"[Sebastian](/{ph}/)", text, flags=re.I) if ph else text
...
def speak_to_bytes(text: str) -> bytes:
    ...
    speed = cfg["tts"]["speed"]
    audio_chunks = []
    for _, _, audio in pipeline(_pronounce(text), voice=voice, speed=speed):
...
def is_speaking() -> bool:
    """Check if Sebastian is currently speaking."""
    ...
    try:
        for sentence in sentences:
            if _interrupt.is_set():
                break
            chunks = []
            for _, _, audio in pipeline(_pronounce(sentence), voice=voice, speed=speed):
                if _interrupt.is_set():
                    break
                if audio is not None:
                    chunks.append(audio)
